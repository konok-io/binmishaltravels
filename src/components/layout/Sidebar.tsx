...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside
      className={`fixed top-0 ${isRTL ? 'right-0' : 'left-0'} h-full bg-white border-gray-200 shadow-sm transition-transform duration-300 z-40 w-64 ${
        sidebarOpen ? 'translate-x-0' : isRTL ? 'translate-x-full' : '-translate-x-full'
      }`}
    >
      {/* Logo */}
      <div className="h-16 flex items-center justify-center border-b border-gray-200 px-4">
        <h1 className="text-xl font-bold text-primary-800">{t('appName')}</h1>
      </div>
