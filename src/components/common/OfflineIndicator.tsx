...
import { getSyncStatus, addSyncListener, syncNow, type SyncStatus } from '@/lib/sync';
...
export const OfflineIndicator: React.FC = () => {
  ...
  const totalPending = status.pendingCount + status.failedCount;
  ...
  return (
    <>
      ...
      {/* Sync Status Button */}
      {totalPending > 0 && status.isOnline && (
        <button
        ...
        >
          ...
          <span className="font-medium">
            {status.isSyncing ? t('syncing') : `${totalPending} ${t('pendingSync')}`}
          </span>
        </button>
      )}
          ...
          <div className="p-4 space-y-3">
            ...
            {status.failedCount > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-red-600">{status.failedCount}</span>
              </div>
            )}
            {status.conflicts > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Conflicts:</span>
                <span className="font-medium text-yellow-600">{status.conflicts}</span>
              </div>
            )}
            <div className="flex justify-between text-sm">
      ...
      {status.isOnline && totalPending === 0 && (
